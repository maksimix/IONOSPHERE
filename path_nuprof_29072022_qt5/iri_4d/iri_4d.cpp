...
void IRI_3D::ioncorrTec(double lat, double lon, double time, double h_tec, double &tec, double &tec_top, double &w, double &ig, int jf)
//
{
  int i;
  float *out_local, *oarr_local;
  int   *jf_local;
  float z1=50., z2=h_tec, zStep=5.;
  float hbeg=50., hend=(float)h_tec;
  float tU;
  int   mes = imd/100;
  ...
  iri_web_(&jmag, &jf_local[0], &lat_f, &lon_f, &iyear, &imd, &iut, &tU,
           &hbeg, &hend, &ivar, &z1, &z2, &zStep, &out_local[0], &oarr_local[0]);
  //iri_web_(&jmag, &jf_local[0], &lat_f, &lon_f, &iyear, &imd, &iut, &tU,
  //         &hbeg, &hend, &ivar, &z1, &z2, &zStep, &a[0], &b[0]);

  tec=oarr_local[36];
  tec_top=oarr_local[37];
  ig=oarr_local[38];
  w =oarr_local[32];
...
//VGG
{
  ...
  if (!iri_3dbase && !iri_3dbase2 && iri_NTime>1)
  {
      int i=0;
      if (time>=iri_tTime[0] && time<=iri_tTime[iri_NTime-1])
      {
          int n=iri_NTime-1;
          for(int j=0; j<n; j++)
          {
              if (time>=iri_tTime[j] && time<iri_tTime[j+1])
              {
                  i=j;
                  break;
              }
...
//VGG
{
  ...
  z2=zMin+10.*dz_f;
  z1=zMin;
  //iri_sub_((int*)&jf[0], (int *)&jmag, &lat, &lon, (int *)&iyea, (int *)&im, &hou, &z1, &z2, &dZ, &out[0], &oarr[0]);
  iri_sub_(&jf[0], &jmag, &lat, &lon, &iyea, (int *)&im, &hou, &z1, &z2, &dz_f, &out[0], &oarr[0]);
}
